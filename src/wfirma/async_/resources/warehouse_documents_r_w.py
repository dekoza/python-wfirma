"""Warehouse document (RW) resource endpoints (asynchronous).
...
class WarehouseDocumentRWResource:
    """Asynchronous resource for RW warehouse documents.
    ...
    async def get(self, warehouse_document_id: int) -> WarehouseDocument:
        ...
        """
        data = await self._client.get_json(f"/warehouse_document_r_w/get/{warehouse_document_id}")
    ...
    async def find(self) -> list[WarehouseDocument]:
        ...
        """
        data = await self._client.get_json("/warehouse_document_r_w/find")
    ...
    async def add(self, document: dict[str, Any]) -> WarehouseDocument:
        ...
        data = await self._client.post_json("/warehouse_document_r_w/add", data=payload)
    ...
    async def delete(self, warehouse_document_id: int) -> bool:
        ...
        """
        await self._client.delete_json(f"/warehouse_document_r_w/delete/{warehouse_document_id}")
        return True
