"""Translation language-related resource endpoints (asynchronous).
...
class TranslationLanguagesResource:
    """Asynchronous translation_languages resource."""

    def __init__(self, client: WFirmaClient) -> None:
        self._client = client

    async def get(self, translation_language_id: int) -> dict[str, Any]:
        ...
        """
        data = await self._client.get_json(f"/translation_languages/get/{translation_language_id}")
    ...
    async def find(self) -> list[dict[str, Any]]:
        ...
        """
        data = await self._client.get_json("/translation_languages/find")
