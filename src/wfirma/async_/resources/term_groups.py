"""Term group-related resource endpoints (asynchronous).
...
class TermGroupsResource:
    """Asynchronous term groups resource."""

    def __init__(self, client: WFirmaClient) -> None:
        self._client = client

    async def add(self, term_group: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        data = await self._client.post_json("/term_groups/add", data={"term_group": term_group})
    ...
    async def find(self, params: dict[str, Any] | None = None) -> list[dict[str, Any]]:
        ...
        """
        data = await self._client.get_json("/term_groups/find", params=params)
        payloads = extract_object_list_payloads(
            data, container_key="term_groups", object_key="term_group"
        )
        return [dict(payload) for payload in payloads]

    async def get(self, term_group_id: int) -> dict[str, Any]:
        ...
        """
        data = await self._client.get_json(f"/term_groups/get/{term_group_id}")
        return self._extract_term_group_payload(data)

    async def edit(self, term_group_id: int, term_group: dict[str, Any]) -> dict[str, Any]:
    ...
    async def delete(self, term_group_id: int) -> dict[str, Any]:
        """Delete a term group.
        ...
        Returns:
            API response dict.
        """
        return await self._client.delete_json(f"/term_groups/delete/{term_group_id}")
