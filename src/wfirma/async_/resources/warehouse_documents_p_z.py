...
class WarehouseDocumentPZResource:
    """Asynchronous resource for PZ warehouse documents.
    ...
    async def get(self, warehouse_document_id: int) -> WarehouseDocument:
        """Get PZ warehouse document by ID.

        Endpoint: GET /warehouse_document_p_z/get/{warehouseDocumentId}
        """
        data = await self._client.get_json(f"/warehouse_document_p_z/get/{warehouse_document_id}")
    ...
    async def find(
        ...
        *,
        ...
        """Find/list PZ warehouse documents.

        Endpoint: GET /warehouse_document_p_z/find
        ...
        if conditions is None and limit is None and page is None:
            data = await self._client.get_json("/warehouse_document_p_z/find")
        else:
    ...
    async def add(self, document: dict[str, Any]) -> WarehouseDocument:
        """Create a new PZ warehouse document.

        Endpoint: POST /warehouse_document_p_z/add
        ...
        data = await self._client.post_json("/warehouse_document_p_z/add", data=payload)
    ...
    async def edit(self, warehouse_document_id: int, document: dict[str, Any]) -> WarehouseDocument:
        """Update an existing PZ warehouse document.

        Endpoint: POST /warehouse_document_p_z/edit/{warehouseDocumentId}
        ...
        data = await self._client.post_json(
            f"/warehouse_document_p_z/edit/{warehouse_document_id}",
            data=payload,
        )
        result_payload = self._extract_document_payload(data)
        return WarehouseDocument.model_validate(result_payload)
    ...
    async def delete(self, warehouse_document_id: int) -> bool:
        """Delete PZ warehouse document.

        Endpoint: DELETE /warehouse_document_p_z/delete/{warehouseDocumentId}
        """
        await self._client.delete_json(f"/warehouse_document_p_z/delete/{warehouse_document_id}")
        return True
