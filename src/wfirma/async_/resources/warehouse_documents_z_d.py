"""Warehouse document (ZD) resource endpoints (asynchronous).
...
class WarehouseDocumentZDResource:
    """Asynchronous resource for ZD warehouse documents.
    ...
    async def get(self, warehouse_document_id: int) -> WarehouseDocument:
        ...
        """
        data = await self._client.get_json(f"/warehouse_document_z_d/get/{warehouse_document_id}")
    ...
    async def find(
        ...
        *,
        ...
        if conditions is None and limit is None and page is None:
            data = await self._client.get_json("/warehouse_document_z_d/find")
        else:
    ...
    async def add(self, document: dict[str, Any]) -> WarehouseDocument:
        ...
        data = await self._client.post_json("/warehouse_document_z_d/add", data=payload)
    ...
    async def delete(self, warehouse_document_id: int) -> bool:
        ...
        """
        await self._client.delete_json(f"/warehouse_document_z_d/delete/{warehouse_document_id}")
        return True
