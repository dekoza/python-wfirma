"""Webhook-related resource endpoints (asynchronous).
...
class WebhooksResource:
    ...
    async def get(self, webhook_id: int) -> dict[str, Any]:
        ...
        """
        data = await self._client.get_json(f"/webhooks/get/{webhook_id}")
        return self._extract_webhook_payload(data)

    async def trigger(self, webhook_id: int) -> dict[str, Any]:
        """Trigger a webhook.
        ...
        """
        return await self._client.get_json(f"/webhooks/trigger/{webhook_id}")
    ...
    async def find(self) -> list[dict[str, Any]]:
        ...
        Returns:
            List of raw webhook payloads.
        """
        data = await self._client.get_json("/webhooks/find")
        payloads = extract_object_list_payloads(
            data, container_key="webhooks", object_key="webhook"
        )
        return [dict(payload) for payload in payloads]

    async def add(self, *, name: str, url: str | None = None) -> dict[str, Any]:
        ...
        if url is not None:
            webhook["url"] = url
        data = await self._client.post_json("/webhooks/add", data={"webhook": webhook})
        return self._extract_webhook_payload(data)

    async def edit(
        self, webhook_id: int, *, name: str | None = None, url: str | None = None
    ) -> dict[str, Any]:
        ...
        if url is not None:
            webhook["url"] = url
        data = await self._client.patch_json(
            f"/webhooks/edit/{webhook_id}", data={"webhook": webhook}
        )
        return self._extract_webhook_payload(data)
    ...
    async def delete(self, webhook_id: int) -> bool:
        ...
        """
        await self._client.delete_json(f"/webhooks/delete/{webhook_id}")
        return True
