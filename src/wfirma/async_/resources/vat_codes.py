"""VAT codes-related resource endpoints (asynchronous).
...
class VatCodesResource:
    """Asynchronous vat_codes resource."""

    def __init__(self, client: WFirmaClient) -> None:
        self._client = client

    async def get(self, vat_code_id: int) -> dict[str, Any]:
        """Get VAT code by ID.
        ...
        """
        data = await self._client.get_json(f"/vat_codes/get/{vat_code_id}")
    ...
    async def find(
        ...
        *,
        ...
        if conditions is None and limit is None and page is None:
            data = await self._client.get_json("/vat_codes/find")
        else:
            parameters = build_find_parameters(conditions, limit=limit, page=page)
            data = await self._client.post_json(
                "/vat_codes/find",
                data={"vat_codes": {"parameters": parameters}},
            )
