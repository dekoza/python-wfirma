"""Invoice delivery-related resource endpoints (synchronous).
...
class InvoiceDeliveriesResource:
    """Synchronous invoice deliveries resource."""
    ...
    def find(
        ...
        *,
        ...
        if conditions is None and limit is None and page is None:
            data = self._client.get_json("/invoice_deliveries/find", params=params)
        else:
    ...
    def get(self, invoice_delivery_id: int) -> dict[str, Any]:
        ...
        """
        data = self._client.get_json(f"/invoice_deliveries/get/{invoice_delivery_id}")
        return self._extract_invoice_delivery_payload(data)
    ...
    def delete(self, invoice_delivery_id: int) -> dict[str, Any]:
        ...
        """
        data = self._client.delete_json(f"/invoice_deliveries/delete/{invoice_delivery_id}")
        return self._extract_invoice_delivery_payload(data)
