"""Synchronous authentication helpers.

This module provides authentication mechanisms for wFirma API:
...
from wfirma.oauth._authlib import SyncOAuth2Backend
...
class OAuth2Auth:
    ...
    def _backend(self) -> SyncOAuth2Backend:
        return SyncOAuth2Backend(
            client_id=self.client_id,
    ...
    def exchange_code(self, code: str) -> OAuthToken:
        ...
        try:
            token = self._backend().exchange_code(code=code)
    ...
    def _refresh(self, refresh_token: str) -> OAuthToken:
        try:
            token = self._backend().refresh(refresh_token=refresh_token)
        except httpx.HTTPStatusError as err:
            if self.debug:
                logger.exception(
...
class OAuth1Auth:
    ...
    def get_headers(
        ...
        *,
        ...
        This builds the OAuth 1.0a ``Authorization`` header using the token stored
        under ``store_key``.
