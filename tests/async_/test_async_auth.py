...
class TestOAuth2Auth:
    ...
    async def test_exchange_code_wraps_timeout_error(self) -> None:
        auth = OAuth2Auth(
            client_id="cid",
            client_secret="csecret",
            redirect_uri="https://app.local/callback",
    ...
    async def test_exchange_code_wraps_request_error(self) -> None:
        auth = OAuth2Auth(
            client_id="cid",
            client_secret="csecret",
            redirect_uri="https://app.local/callback",
    ...
    async def test_refresh_wraps_http_status_error(self) -> None:
        auth = OAuth2Auth(
            client_id="cid",
            client_secret="csecret",
            redirect_uri="https://app.local/callback",
        ...
        with patch.object(
            ...
            await auth._refresh("refresh-token")
...
class TestOAuth1Auth:
    # AICOMPLETE: OAuth1 async flow basics (request/access token, auth URL, headers) - ready for review
    ...
    async def test_get_headers_contains_expected_authorization_header(self) -> None:
        auth = OAuth1Auth(
            consumer_key="ck",
            consumer_secret="cs",
            scope="invoices",
    ...
    async def test_get_headers_raises_validation_error_when_token_secret_missing(self) -> None:
        auth = OAuth1Auth(
            consumer_key="ck",
            consumer_secret="cs",
            scope="invoices",
    ...
    async def test_get_headers_raises_when_token_missing(self) -> None:
        auth = OAuth1Auth(
            consumer_key="ck",
            consumer_secret="cs",
            scope="invoices",
