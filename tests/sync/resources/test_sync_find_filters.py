"""Filtered ``find`` tests for synchronous resources.

Every wFirma ``find`` endpoint accepts a ``parameters`` block (conditions,
limit, page — doc.wfirma.pl, "Format wymiany danych"). These tests assert
that each resource exposes those filters and serializes them into the
documented request body:

    {"tags": {"parameters": {"conditions": {"0": {"condition": {...}}},
                             "limit": 5, "page": 2}}}

An unfiltered ``find()`` must keep using a plain GET request.
"""
...
class TestFilteredFind:
    ...
    def test_find_with_filters_posts_parameters_block(
        ...
        with respx.mock:
            ...
            result = getattr(client, accessor).find(conditions=CONDITIONS, limit=5, page=2)
        client.close()
    ...
    @pytest.mark.parametrize(("accessor", "slug", "container"), FIND_RESOURCES)
    def test_find_without_filters_uses_get(self, accessor: str, slug: str, container: str) -> None:
        client = _client()
        with respx.mock:
            route = respx.get(f"https://api2.wfirma.pl/{slug}/find").mock(
                return_value=_empty_response(container)
            )
            result = getattr(client, accessor).find()
        client.close()
